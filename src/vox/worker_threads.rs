...
pub fn spawn_chunk_creation_loop(
    ...
    Sender<(i32, i32)>,
    Receiver<(i32, i32, Arc<RwLock<Chunk>>)>
) {
    ...
    thread::spawn(move || {
        ...
        loop {
            let data: (usize, i32, i32, Arc<RwLock<Chunk>>) = worker_recv_finished_chunks.recv().unwrap();
            let id = data.0.clone();
...
pub fn spawn_chunk_meshing_loop(
    ...
    thread::spawn(move || {
        let send_idle_worker = send_idle_worker.clone();
        loop {
            let data: (usize, i32, i32, u32, ((Vec<SurfaceVertex>, Vec<u32>, u32), (Vec<SurfaceVertex>, Vec<u32>, u32))) = worker_recv_finished_chunks.recv().unwrap();
            let id = data.0.clone();
