...
pub struct InputService {
     ...
     */
    pub on_key_pressed: MonoThreadSignal<(KeyCode, bool)>,
    /**
     * first parameter: the key
     * second parameter: whether or not the click was absorbed by ui
     */
    pub on_key_released: MonoThreadSignal<(KeyCode, bool)>,
    /**
     ...
     * second parameter: whether or not the click was absorbed by ui
     */
    pub on_mouse_released: MonoThreadSignal<(MouseButton, bool)>,
...
unsafe impl Send for InputService {}
unsafe impl Sync for InputService {}
...
impl InputService {
    pub fn new(window: Arc<Window>) -> Self {
        Self {
            ...
            on_mouse_released: MonoThreadSignal::new(),
        ...
        match state {
            ElementState::Pressed => {
                if !self.mouse_states.contains_key(&btn) {
                    ...
                    self.on_mouse_click.dispatch(((*btn).into(), consumed)).await;
                }
            },
            ElementState::Released => {
                if self.mouse_states.contains_key(&btn) {
                    self.mouse_states.remove(&btn);
                    self.on_mouse_released.dispatch(((*btn).into(), consumed)).await;
                }
            },
        }
    }
                ...
                match k.state {
                    winit::event::ElementState::Pressed => {
                        if !self.key_states.contains_key(&code) {
                            ...
                            self.on_key_pressed.dispatch((code, consumed)).await;
                        }
                    },
                    winit::event::ElementState::Released => {
                        if self.key_states.contains_key(&code) {
                            self.key_states.remove(&code);
                            self.on_key_released.dispatch((code, consumed)).await;
                        }
                    },
                }
            },
